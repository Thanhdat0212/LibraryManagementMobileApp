...
export const bookCopiesApi = {
  ...
  getById: (id: number) => axiosClient.get<BookCopy>(`/bookcopies/${id}`).then((res) => res.data),
  create: (data: CreateBookCopyRequest) =>
    axiosClient.post<BookCopy>("/bookcopies", data).then((res) => res.data),
  update: (id: number, data: UpdateBookCopyRequest) =>
    axiosClient.put(`/bookcopies/${id}`, data),
